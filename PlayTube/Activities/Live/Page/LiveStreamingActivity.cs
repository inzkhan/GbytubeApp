...
namespace PlayTube.Activities.Live.Page
{
    ...
    public class LiveStreamingActivity : RtcBaseActivity, IEventHandler , MaterialDialog.IListCallback/*, PixelCopy.IOnPixelCopyFinishedListener*/
    {
        ...
        ////////////////////////////////
        private VideoObject LiveStreamViewerObject;
        private VideoObject PostObject;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                {
                    case false:
                        LiveStreamViewerObject = JsonConvert.DeserializeObject<VideoObject>(Intent?.GetStringExtra("PostLiveStream") ?? "");
                        VideoId = LiveStreamViewerObject.Id;
                        break;
                }
        ...
        { 
            try
            {
                ...
                if (isStreamer)
                {
                    if (ListUtils.MySettingsList?.LiveVideoSave != null && ListUtils.MySettingsList?.LiveVideoSave.Value == 0) 
                        DeleteLiveStream();
                    else
                    {
                    ...
                    if (apiStatus == 200)
                    {
                        if (respond is MessageIdObject result)
                        {
                            ...
                            if (date != null)
                            {
                                ...
                                index = MAdapter.CommentList.IndexOf(MAdapter.CommentList.FirstOrDefault(a => a.Id == Convert.ToInt32(time2)));
                                if (index > -1)
                                {
        ...
        private async Task CreateLive()
        {
            ...
            var (apiStatus, respond) = await RequestsAsync.Live.CreateLive(Config().GetChannelName());
            if (apiStatus == 200)
            {
        ...
        private void DeleteLiveStream()
        {
            if (!Methods.CheckConnectivity())
                ...
                PollyController.RunRetryPolicyFunction(new List<Func<Task>> { async () => await RequestsAsync.Live.DeleteLive(VideoId) });
        }
        ...
        { 
            if (Methods.CheckConnectivity())
            {
                ...
                var (apiStatus, respond) = await RequestsAsync.Live.CheckCommentsLive(VideoId, IsOwner ? "live" : "story", "10", offset, resourceId, sid, fileList);
